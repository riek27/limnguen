...
function verifyPassword(candidate: string, stored: any): boolean {
  ...
  if (stored?.account?.passwordHash) return hashed === stored.account.passwordHash;
  return hashed === hashPassword('Foundation2019');
}
...
export async function POST(request: Request) {
  try {
    ...
    const updatedAccount: any = {
      username: (newUsername && newUsername.trim()) || existing?.account?.username || 'limnguen',
    ...
    await savePage('lnf-settings', merged);

    return NextResponse.json({ success: true, username: updatedAccount.username });
  } catch (error: any) {
    return NextResponse.json(
